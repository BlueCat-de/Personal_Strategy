...
这个策略里，当我们没有创建订单时就买入1000股'600570.SS'，具体的下单API请看 [order](https://ptradeapi.com/index.html#order) 函数。这里我们有了交易，但是只是无意义的交易，没有依据当前的数据做出合理的分析。
...
1. 框架会在
  [before_trading_start（隔日开始）](https://ptradeapi.com/index.html#before_trading_start)
  、
  [handle_data](https://ptradeapi.com/index.html#handle_data)
  、
  [after_trading_end](https://ptradeapi.com/index.html#after_trading_end)
  事件后触发持久化信息更新及保存操作；
2. 券商升级/环境重启后恢复交易时，框架会先执行策略
  [initialize](https://ptradeapi.com/index.html#initialize)
  函数再执行持久化信息恢复操作。如果持久化信息保存有策略定义的全局对象g中的变量，将会以持久化信息中的变量覆盖掉
  [initialize](https://ptradeapi.com/index.html#initialize)
  函数中初始化的该变量。
3. 全局变量g中不能被序列化的变量将不会被保存。您可在
  [initialize](https://ptradeapi.com/index.html#initialize)
  中初始化该变量时名字以'__'开头；
