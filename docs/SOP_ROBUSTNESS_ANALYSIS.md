...
**泄漏防护（embargo）**：若把扰动结果按时间分段比对（如"早期 vs 晚期窗口一致性"），段与段之间必须留 **embargo 间隔 ≥ 最长前瞻 horizon**（月频≈21 交易日），剔除跨越分段边界的前瞻收益标签——否则相邻段共享标签 = 信息泄漏。`walk_forward.walk_forward_folds(..., embargo_days=21)` 已内置此机制。
...
**判定要看跨窗口一致性，而非单段**：扰动/消融的 Δ 应在 **walk-forward 多个 OOS 窗口**上一致（见 `walk_forward` 工具），而不是只看 dev+val 这一段。dev+val 同向只是必要条件，不是充分条件——v2 的 marginal ablation 在 dev+val 都"同意"砍掉行为因子，但那些因子在后续 test 才显现稳定性，正是"两段一致"仍过拟合的反例。

**合法的结论类型**：
1. **脆弱性告警**：某维度小幅扰动 → 大幅退化。实盘前必须知道，但不一定要改（可能只是边界）。
2. **冗余简化**：消融在**多个 walk-forward OOS 窗口同时改善** → 去掉该因子是稳健的简化。**任何被采纳的简化都计入 DSR 的 N**（你试了"砍/不砍"= 多试了一个策略变体）。
3. **成本可行性**：滑点在合理范围内不崩 → 实盘可行。
4. **参数任意性**：某参数邻域平缓 → 该参数选择不敏感，实盘放心；某参数是孤立峰 → 实盘要严格还原。

**非法的结论**：基于 val 单段（甚至 dev+val 两段）的最优去反向修改权重/参数，再声称「更好」。

**消融 ≠ 选择（与 SOP_STRATEGY_DEVELOPMENT §3.2 对齐）**：leave-one-out 消融只回答"当前权重下某因子的边际贡献"，**不**是因子选择工具。因子选择必须用逐年 IC 稳定性，在 train 折内做（嵌套）。把消融结果直接用于"砍因子入策略"= 在 val 上选→过拟合。
