...
取数任务完成后，策略任务每天 16:50 运行。脚本会先检查：

```text
data/offline/a_share_12m_tencent_sina/prices_long.csv
```

最新日期是否等于当天。如果数据没有更新到当天，策略不会运行，并会飞书说明“今日数据未更新，跳过策略”。
...
- 每日飞书收益是“严格按策略从 `start-date` 跟随”的模型收益，不会自动读取真实券商账户。
